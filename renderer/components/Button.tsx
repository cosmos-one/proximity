...
export const Button: React.FC<ButtonProps> = ({button, submit, disabled, type}) => {
    return (
        <>
        {
            ...
            :
            (
                <button disabled={disabled || false} className={`p-1 border border-green rounded-md ${ disabled ? "opacity-50" : "hover:bg-green hover:text-black hover:cursor-pointer"} duration-150`} onClick={(e) => {submit ? submit(e) : null}}>
                    {button}
                </button>
            )
        }
